...
OUTPUT_ROOT="${PROJECT_ROOT}/runs/icra_ros_physical"
CHECKPOINT="${PROJECT_ROOT}/runs/icra2027_physical_anchor_seed0/checkpoints/best.pt"
...
run_trial() {
  ...
  if ! timeout 15 rostopic echo -n 1 /sim/odom >/dev/null 2>&1; then
    echo "No odometry for ${goal_id}/${variant}" >&2
    return 1
  fi
  if ! timeout 15 rostopic echo -n 1 /depth_image >/dev/null 2>&1; then
    echo "No depth image for ${goal_id}/${variant}" >&2
    return 1
  fi
...
for variant in physical_raw physical_selector; do
...
echo "Physical-anchor ROS ablation complete: ${OUTPUT_ROOT}/ROS_SUMMARY.md"
